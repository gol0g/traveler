...
미국(NYSE/NASDAQ) 및 한국(KOSPI/KOSDAQ) 주식에서 기술적 분석 패턴을 탐지하고, 한국투자증권 API를 통해 자동 매매하는 Go CLI 프로그램입니다.

## 주요 기능

- **멀티 전략 스캔**: Pullback, Breakout, Mean Reversion 3종 전략 동시 지원
- **듀얼 마켓**: 미국 + 한국 주식 동시 지원 (USD/KRW)
- **자동 매매**: 한국투자증권(KIS) API 해외/국내 주식 연동
- **포트폴리오 관리**: 계좌 잔고 기반 포지션 사이징
- **리스크 관리**: 자동 손절/익절 모니터링
- **Web UI**: 브라우저 기반 스캔, 차트 분석, US/KR 마켓 토글
- **Daemon 모드**: PC 자동 기상 → 매매 → 절전 완전 자동화
...
### 3. Mean Reversion (평균 회귀)
```
과매도 반등 매매:
- RSI 30 이하 (과매도 구간)
- 볼린저 밴드 하단 이탈
- MA200 위에서 단기 급락 (장기 추세 건재)
- 역추세 반등 매매용
```
...
- **US/KR 토글**: 마켓 전환
- **Scan**: 실시간 멀티 전략 스캔
- **Positions**: 실계좌 포지션 확인
- **Detail**: 개별 종목 차트 및 매매 가이드

### 자동 매매 (KIS API 연동)
```bash
# Dry-run 모드 (실제 주문 안함)
./traveler --strategy all --universe russell --auto-trade --dry-run

# 실전 매매 (CONFIRM 입력 필요)
./traveler --strategy all --universe russell --auto-trade --dry-run=false
```
...
kis:
  ...
  # 국내주식 계좌 (별도 AppKey인 경우)
  domestic:
    app_key: "YOUR_KR_APP_KEY"
    app_secret: "YOUR_KR_APP_SECRET"
    account_no: "12345678-01"
...
# 국내주식
export KIS_KR_APP_KEY="your_kr_key"
export KIS_KR_APP_SECRET="your_kr_secret"
export KIS_KR_ACCOUNT_NO="12345678-01"
```
...
| `--data-dir` | ~/.traveler | 데이터 디렉토리 (plans, logs, reports) |
| `--verbose` | false | 상세 출력 |
...
| `--adaptive` | false | 적응형 스캔 (잔고 기반 유니버스 선택) |
...
|------|----------|-------------|------------|-------------|
| < ₩50만 | kosdaq30 → kospi30 | 3% | 40% | 3개 |
| < ₩500만 | kosdaq30 → kospi30 | 2% | 30% | 3개 |
| < ₩5000만 | kospi30 + kosdaq30 → kospi200 | 1.5% | 25% | 5개 |
| ≥ ₩5000만 | kospi30 + kosdaq30 → kospi200 | 1% | 20% | 5개 |
...
### 매매 플로우
```
1. KIS API 토큰 확인 (AppKey별 캐시 또는 신규 발급)
2. 계좌 잔고 조회
3. 유니버스 멀티 전략 스캔 → 시그널 수집
...
### 동작 흐름
```
1. 시작 → 마켓 상태 확인 (US: ET 기준, KR: KST 기준)
2. 마켓 닫힘 → 오픈까지 대기
3. 마켓 오픈 → 적응형 멀티 전략 스캔 (30분 주기)
...
| API | Rate Limit | 용도 |
|-----|------------|------|
| **KIS 국내** | 분당 300회 | 한국 주식 시세 + 매매 |
| **Finnhub** | 분당 60회 | 미국 주식 시세 |
| **Yahoo Finance** | 비공식 | 미국 주식 폴백 (API 키 불필요) |
...
│   │   └── kis/                  # 한국투자증권 API
│   │       ├── client.go         # HTTP 클라이언트 (해외/국내 듀얼)
│   │       ├── auth.go           # OAuth 토큰 관리 (AppKey별 캐시)
│   │       └── types.go          # 요청/응답 타입 (해외+국내)
│   ├── daemon/                   # 데몬 모드 (완전 자동화)
│   │   ├── daemon.go             # 메인 오케스트레이터
│   │   ├── market.go             # US/KR 마켓 시간 관리
...
│   │   ├── adaptive.go           # 적응형 스캔 (US/KR 티어)
│   │   ├── planstore.go          # 매매 계획 저장/관리
│   │   └── risk.go               # 리스크 관리
│   ├── strategy/                 # 매매 전략
...
│   ├── provider/                 # 시세 데이터 API
│   │   ├── finnhub.go            # Finnhub (US)
│   │   ├── yahoo.go              # Yahoo Finance (US)
│   │   ├── kis.go                # KIS 국내주식 시세
│   │   ├── caching.go            # 캐싱 프로바이더
...
- [x] 멀티 전략 동시 실행 (pullback + breakout + mean-reversion)
- [x] 한국 국내주식 지원 (KOSPI/KOSDAQ)
- [x] Web UI 듀얼 마켓 토글 (US/KR)
- [ ] 모니터 자동 켜기 (wake 후)
